...
def preprocess_data(data):
    ...
    # Count spam keywords in content
    ...
    data['CONTENT_FLTR'] = data['CONTENT'].str.lower().apply(lambda x: [item for item in x.split() if item not in sr])

    # Calculate comment length
    data['COMMENT_LEN'] = data['CONTENT_FLTR'].str.len()

    # Calculate ratio of spam keywords to comment length
    data['SPM_to_COMNT'] = data['SPM_CNT'] / data['COMMENT_LEN']

    # Set flag for comment length greater than 50
    data['spm_len'] = np.where(data['COMMENT_LEN'] > 50, 1, 0)

    # Count number of words in content
    data['WORD_COUNT'] = data['CONTENT_FLTR'].apply(len)

    # Count number of characters in content
    data['CHARACTER_COUNT'] = data['CONTENT'].apply(len)

    # Check if content contains numbers
    data['CONTAINS_NUMBERS'] = data['CONTENT'].str.contains(r'\d', regex=True).astype(int)

    # Count number of emojis in content
    data['CONTAINS_EMOJIS'] = data['CONTENT'].apply(lambda x: emoji.emoji_count(x))

    # Check if content contains punctuation
    data['CONTAINS_PUNCTUATION'] = data['CONTENT'].apply(lambda x: any(char in string.punctuation for char in x)).astype(int)

    # Check if content contains all capital letters
    data['CONTAINS_ALL_CAPS'] = data['CONTENT'].apply(lambda x: any(word.isupper() for word in x.split())).astype(int)
...
def classify():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            X_unlabeled = unclass_data[['SPM_CNT', 'IS_URL', 'COMMENT_LEN', 'SPM_to_COMNT', 'spm_len', 'WORD_COUNT', 'CHARACTER_COUNT', 'CONTAINS_NUMBERS', 'CONTAINS_EMOJIS', 'CONTAINS_PUNCTUATION', 'CONTAINS_ALL_CAPS']].values
